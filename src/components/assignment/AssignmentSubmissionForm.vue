    ...
    <div>
      ...
      <div class="flex-auto px-6">
        ...
        <div class="flex flex-wrap -mx-3">
          <TextInput disabled class="md:w-6/12" label="Group Name / Student Name"
                     v-model="model.name"></TextInput>
          <DateInput disabled time class="md:w-6/12" label="Submission Date"
                     v-model="model.submissionDate"></DateInput>
        </div>
        ...
        <div v-for="(x, index) in model.groupMember" :key="index"
            ...
            <TextArea disabled v-model="x.name" class="md:w-8/12"
                      :label="'Member Name ' + (index+1) "></TextArea>
          <TextInput :disabled="props.disabled" v-model="x.score" class="md:w-3/12 h-full"
                     :label="'Member score ' + (index+1)"></TextInput>
          <Button v-if="!(props.disabled)" class="md:w-1/12 font-semibold text-blue-500"
                  @click="grade(x)">
